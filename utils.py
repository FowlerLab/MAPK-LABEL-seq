...
#: Our re-scored table -- the base layer for the figures since 2026-08-14. Built
#: by scripts/score_cell.py + finalize_scores.py from our own barcode counts, and
#: carrying Jessica's structural annotations transferred at position level. It is
#: a drop-in for JESSICA_MASTERFRAME: every column Annotations.ipynb consumes is
#: present, and the five chaperone columns it lacks are ones the notebook drops
#: anyway. Use this, not her scores -- see docs/count_provenance_audit.md for the
#: provenance fixes that make our scores the better base (all 64 cells now at
#: rho >= 0.90 against her masterframe, median 0.994).
...
#: scripts/compute_structure_annotations.py over the AlphaFold models). Nothing
#: in it is transferred from Jessica's delivery.
SCORES_REANNOTATED = OUTPUT / "scoring" / "scores_reannotated.tsv"
#: Jessica's delivered scoring + structural annotation table. No longer the base
#: layer; retained as the reference the audit scripts compare against.
JESSICA_MASTERFRAME = DATA / "Annotated_Replicate_scores_masterframe_autogenerated_080126.tsv"
#: Our previous end-to-end output. Used only to splice RET's HSP90i abundance,
#: which is a duplicate of its control in the delivered file (2026-08-02).
LEGACY_MASTERFRAME = OUTPUT / "Annotated_Replicate_scores_masterframe.tsv"
#: Written by Annotations.ipynb: Jessica's scores with RET HSP90i repaired.
SCORES_REPAIRED = OUTPUT / "scores_masterframe_repaired.tsv"
...
#: Optional. Jessica's per-library empty-vector score summary, needed for the
#: bootstrap boxplot in Fig 3A. Absent from the repo as of 2026-08-02 — no raw
#: file contains barcode-level ``empty_vector_std``, so the distribution cannot
#: be recomputed here. Panels degrade gracefully when this is missing.
EV_SCORE_STATS = DATA / "empty_vector_score_stats_autogenerated.tsv"
...
#: The AlphaFold v4 monomer models those are computed from, and which Jessica's
#: DSSP (hence every RSA in the project) also came from.
AF_STRUCTURES = (PROJECT_ROOT / "jessica_analysis" / "260404_files_for_sri"
                 / "Annotating scores" / "AF_MAPK_structures")

#: Quantitative predictors and contact sets, each produced by its own script.
SPURS_DDG = PROJECT_ROOT.parent / "claude_SPURS" / "spurs_ddg_all_proteins.tsv"
...
#: Activity-class palette carried over from Jessica's notebooks so our panels
#: sit beside hers without a colour clash.
ACTIVITY_COLORS = {
    "DN": "#C0392B",
    "low": "#26A69A",
    "wt-like": "#F4E04D",
